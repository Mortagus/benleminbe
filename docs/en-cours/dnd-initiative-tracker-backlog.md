...
Ce document remplace l'ancien backlog d'audit et la roadmap avancée. Il conserve les tâches déjà réalisées, aligne les fonctionnalités de la roadmap avec l'état réel du projet et
liste les évolutions restantes.
...
Les commandes explicites de pilotage du combat `P7` sont désormais livrées.

Travail livré :

- ajout des commandes `Acteur suivant`, `Nouveau round`, `Réinitialiser les tours de ce round` et `Réinitialiser la rencontre` ;
- affichage lisible du round courant, de l'acteur actif et de l'état round terminé ;
- reset global déplacé dans l'en-tête de page pour signaler clairement son périmètre ;
- reset de la rencontre persistant comme état vide afin d'empêcher la restauration d'une ancienne rencontre après reload ;
- tests Vitest pour les transitions de combat, l'état du panneau et la persistance du reset.
...
- ne pas traiter les DTOs secondaires maintenant : champ/résultat de validation et groupes d'options monstres restent des structures locales à revoir plus tard si le besoin
  augmente ;
...
- séparation du mapping joueur en deux étapes : `readPlayerForm()` lit les champs `data-player-field`, puis `createPlayerActor()` produit la structure consommée par
  `EncounterState` ;
- mise à jour de la cartographie JS.
...
- la prochaine passe de lisibilité recommandée était `validation.js`, car ce module mélangeait encore règles de validation, inspection DOM, affichage des erreurs et gestion du
  focus ;
...
Une fois le cycle Lighthouse/cache et la validation de la zone privee termines, le prochain projet hors partie professionnelle du site est de reprendre l'amelioration du
...
Le backlog est organisé sous forme de tableau pour pouvoir réordonner les points sans déplacer de longues sections descriptives. La colonne `Ordre conseillé` donne l'enchaînement
actuel recommandé.

Les points d'architecture, de modèle de données, de tests et de contrats techniques sont volontairement remontés quand ils conditionnent plusieurs fonctionnalités futures.
L'objectif est d'éviter de construire les commandes de combat, la sauvegarde ou les PV en direct sur des fondations fragiles.
...
| 20              | P10 | Réalisé           | Fait                  | Fonctionnalité / UX        | Modification directe des PV pendant le combat            | Modifier les PV depuis l'ordre du tour et accepter des saisies rapides de dégâts/soins comme `-7` ou `+5`.                                                                                  | Préparer sans imposer immédiatement les statuts inconscient/mort.                                                                                |
| 21              | P19 | Haute             | Fait                  | Fonctionnalité / Règles    | Gestion des conditions et des états de combat            | Ajouter/retrouver des conditions, les afficher visuellement, distinguer les états vitaux et suivre leur durée en rounds.                                                                    | Le socle de combat est désormais en place ; la suite peut porter sur les marqueurs binaires, la détection d'issue et le journal.                 |
| 35              | P33 | Haute             | À faire               | Règles / Joueurs           | DEX des joueurs saisis à la main                         | Ajouter une saisie de DEX ou de modificateur d'initiative pour les joueurs créés manuellement, afin que le départage d'égalité par DEX fonctionne pour tout le monde.                       | Complète `P24` sans dépendre de l'import XML ; à brancher dans le formulaire joueur, la validation et la persistance.                            |
| 22              | P20 | Haute             | À faire               | Fonctionnalité / Règles    | Marqueurs binaires de combat                             | Suivre concentration, réaction utilisée, inspiration, avantage et désavantage via toggles visuels.                                                                                          | À commencer sans automatisme de règles ; peut partager l'UI des conditions et états de combat.                                                   |
| 23              | P35 | Moyenne           | À faire               | Fonctionnalité / Règles    | Détection d'issue de rencontre                           | Détecter quand un camp n'a plus d'acteurs actifs et signaler une issue probable de combat, sans déclarer automatiquement une victoire définitive.                                           | S'appuie sur `P7` pour le pilotage du combat, `P10` pour les PV, `P19` pour les états de combat et `P25` pour les camps ; le MJ reste souverain. |
| 23-bis          | P36 | Moyenne           | À faire               | Joueurs / Fiche personnage | Affichage enrichi de la fiche PJ importée                | Améliorer l’affichage de la fiche personnage issue de l’import XML afin de présenter les informations par sections lisibles et utiles au MJ, plutôt qu’un simple dump technique du payload. | S’appuie sur `P32` et `importData`; prépare une future capacité de mise à jour/évolution du PJ sans imposer l’édition complète maintenant.       |
| 24              | P21 | Moyenne           | À faire               | Fonctionnalité / Suivi     | Journal de combat                                        | Journaliser début de round, changements de PV, acteur joué, conditions ajoutées ou retirées, états modifiés et issue probable de rencontre.                                                 | Dépend du modèle, des commandes de combat, des PV en direct, des conditions/états et peut intégrer plus tard `P35`.                              |
| 25              | P34 | Moyenne           | À faire               | Fonctionnalité / MJ        | Jet de constitution des monstres                         | Permettre au MJ de lancer rapidement un jet de Constitution ou un jet de sauvegarde de Constitution pour un monstre, comme mini-outil d'assistance en combat.                               | Utilitaire de combat ponctuel ; utile après les bases du tracker, sans bloquer `P20`, `P35` ou `P21`.                                            |
| 26              | P14 | Moyenne           | À faire               | UX                         | Retours d'état utiles au Maître du Jeu                   | Afficher des messages courts : aucun monstre sélectionné, initiatives non lancées, joueur incomplet, acteur marqué joué, condition expirée ou issue probable.                               | Peut être livré par petites touches sans attendre les grosses fonctionnalités.                                                                   |
...
| 29              | P17 | Basse             | À faire               | CSS / UX                   | Affichage desktop et tablette sur grands combats         | Optimiser les combats volumineux pour un écran 24 pouces 16:9, puis vérifier les écrans plus petits et tablettes.                                                                           | Mobile hors périmètre immédiat ; à valider avec les nouveaux contrôles de combat, les PV directs, les conditions et les états.                   |
| 30              | P22 | Plus tard         | Plus tard             | Persistance / Échange      | Import/export JSON d'une rencontre                       | Exporter et importer un état de rencontre lisible : joueurs, monstres, PV, initiatives, règles, ordre courant, conditions et états de combat.                                               | À reconsidérer après stabilisation des commandes de combat, de la persistance locale et des états de rencontre.                                  |
| 31              | P27 | Plus tard         | Plus tard             | Fonctionnalité             | Notes par participant                                    | Stocker des notes temporaires pour un joueur, monstre ou effet en cours.                                                                                                                    | À reconsidérer après les besoins réels de suivi de combat.                                                                                       |
| 32              | P28 | Plus tard         | Plus tard             | Player-facing              | Vue joueur                                               | Afficher une vue simplifiée partageable avec les joueurs, sans détails réservés au MJ.                                                                                                      | À envisager après stabilisation de la vue MJ.                                                                                                    |
| 33              | P29 | Plus tard         | Plus tard             | Player-facing              | Mode plein écran                                         | Afficher le tracker sur un écran secondaire ou une TV.                                                                                                                                      | Probablement lié à la future vue joueur.                                                                                                         |
| 34              | P30 | Plus tard         | Plus tard             | Fonctionnalité avancée     | Créatures temporaires et invocations                     | Gérer les créatures invoquées ou temporaires avec un cycle de vie plus court qu'un participant classique.                                                                                   | À traiter après clarification des conditions, rounds, états de combat et templates de rencontre.                                                 |
...
| Ordre du tour | Avancer au prochain acteur                                           | Fait                  | Marque l'acteur courant joué, avance au suivant non joué et signale le round terminé sans passage automatique au round suivant. |
| Ordre du tour | Nouveau round                                                        | Fait                  | Incrémente le round courant, remet tous les tours à non joué et conserve l'ordre actuel.                                        |
| Ordre du tour | Réinitialiser les tours de ce round                                  | Fait                  | Remet tous les tours du round courant à non joué sans modifier le round.                                                        |
| Ordre du tour | Réinitialiser la rencontre                                           | Fait                  | Remise à zéro globale de la rencontre avec neutralisation de la sauvegarde locale de l'état de combat.                          |
